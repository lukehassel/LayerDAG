...
def get_universal_gate_set():
    ...
    """
    return {
        'single_qubit': ['h', 'id', 'p', 'rx', 'ry', 'rz', 'sx', 'u', 'x', 'y', 'z'],
        'two_qubit': ['cp', 'cx', 'cz', 'rxx', 'ryy', 'rzz', 'swap'],
        'all': ['cp', 'cx', 'cz', 'h', 'id', 'p', 'rx', 'rxx', 'ry', 'ryy', 'rz', 'rzz', 'swap', 'sx', 'u', 'x', 'y', 'z']
    }
...
def create_circuit_with_all_gates():
    """
    Create a circuit that contains all gates from the universal gate set.

    This ensures all gate types appear at least once, making gate type indices
    consecutive with no gaps. Useful for adding to datasets to guarantee all
    gate types are represented.

    Returns:
        QuantumCircuit: Circuit with all 18 universal gates
    """
    from qiskit.circuit.library import CPhaseGate, RXXGate, RYYGate, RZZGate

    # Get all gates in sorted order (same order as gate_to_idx mapping)
    all_gates = sorted(get_universal_gate_set()['all'])

    # Create circuit with 2 qubits (enough for all gate types)
    num_qubits = 2
    circuit = QuantumCircuit(num_qubits)

    # Map gate names to (method, qubits, params)
    gate_specs = {
        'cp': (lambda p: circuit.cp(p, 0, 1), [0.5]),
        'cx': (lambda: circuit.cx(0, 1), []),
        'cz': (lambda: circuit.cz(0, 1), []),
        'h': (lambda: circuit.h(0), []),
        'id': (lambda: circuit.id(0), []),
        'p': (lambda p: circuit.p(p, 0), [0.5]),
        'rx': (lambda p: circuit.rx(p, 0), [0.5]),
        'rxx': (lambda p: circuit.append(RXXGate(p), [0, 1]), [0.5]),
        'ry': (lambda p: circuit.ry(p, 0), [0.5]),
        'ryy': (lambda p: circuit.append(RYYGate(p), [0, 1]), [0.5]),
        'rz': (lambda p: circuit.rz(p, 0), [0.5]),
        'rzz': (lambda p: circuit.append(RZZGate(p), [0, 1]), [0.5]),
        'swap': (lambda: circuit.swap(0, 1), []),
        'sx': (lambda: circuit.sx(0), []),
        'u': (lambda t, p, l: circuit.u(t, p, l, 0), [0.5, 0.5, 0.5]),
        'x': (lambda: circuit.x(0), []),
        'y': (lambda: circuit.y(0), []),
        'z': (lambda: circuit.z(0), []),
    }

    # Iterate over all gates and add each one
    for gate_name in all_gates:
        gate_method, params = gate_specs[gate_name]
        gate_method(*params)
